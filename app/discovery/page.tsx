...
export default function PlayRequestPage() {
  ...
  // ── تحميل أولي فقط (بدون polling — السوكيت رح يحدّث لايف) ──
  useEffect(() => {
  ...
  // ── Socket listeners (بدل الـ polling) ──
  useEffect(() => {
    if (!socket) return;

    socket.on("message-sent", ({ requestId: rid, message }: any) => {
      if (rid === requestId) setMessages((prev) => [...prev, message]);
    });

    socket.on(
      "player-joined-request",
      ({ requestId: rid, participant }: any) => {
        if (rid === requestId) {
          setPlayRequest((prev) =>
            prev
    ...
    socket.on("player-left-request", ({ requestId: rid, userId }: any) => {
      if (rid === requestId) {
        setPlayRequest((prev) =>
          prev
            ? {
    ...
    socket.on("request-deleted", ({ requestId: rid }: any) => {
      if (rid === requestId) {
    ...
    return () => {
      ...
      socket.off("player-joined-request");
      socket.off("player-left-request");
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessageInput(""); // ✅ الرسالة رح توصل عبر socket "message-sent"
    } catch (error) {
        ...
        <GamingCard glow className="p-12 text-center">
          ...
          <Link href="/discovery">
            <GamingButton variant="accent" glow>
              <ArrowLeft className="w-4 h-4 ml-2" /> العودة للاكتشاف
            </GamingButton>
          </Link>
        </GamingCard>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <motion.div
              ...
              className={`px-4 py-2 rounded-full font-bold text-sm ${isFull ? "bg-red-500/20 text-red-400 border border-red-500/30" : "bg-green-500/20 text-green-400 border border-green-500/30"}`}
            >
              {isFull ? "🔴 ممتلئة" : "🟢 متاحة"}
            </motion.div>
          </div>
              ...
              {!isOwner && !isParticipant && !isFull && (
                ...
                >
                  <GamingButton
                    ...
                    disabled={joiningId === requestId}
                  >
                    <Users className="w-5 h-5 ml-2" />
                    {joiningId === requestId ? "جاري الانضمام..." : "انضم الآن"}
                  </GamingButton>
                </motion.div>
              )}
                    ...
                    {messages.length === 0 ? (
                      ...
                      messages.map((msg) => (
                        <motion.div
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          exit={{ opacity: 0, y: -10 }}
                          className={`flex gap-3 ${msg.user.id === currentUser?.id ? "flex-row-reverse justify-end" : "justify-start"}`}
                        >
                          ...
                          <div
                            className={`max-w-xs rounded-2xl p-4 ${msg.user.id === currentUser?.id ? "bg-gradient-to-br from-cyan-600 to-blue-600 text-white" : "bg-slate-800/70 text-slate-300 border border-slate-700/50"}`}
                          >
